...
DeltaQ[x_]:=If[x===Delta,Return[False];,Return[True];];
 ...
 subRGENoDelta={
      sumRGE[s_,x_,y_]A_[s_,a___]B_[s_,b___]-> MatMul[Tp[A],B][a,b],
      sumRGE[s_,x_,y_]A_[a___,s_]B_[b___,s_]-> MatMul[A,Tp[B]][a,b],
      sumRGE[s_,x_,y_]A_[s_,a___]B_[b___,s_]-> MatMul[B,A][b,a],
      sumRGE[s_,x_,y_]A_[s_,a___]^2-> MatMul[Tp[A],A][a,a],
...
MatMul[a_,b_,c_] := 
    MatMul[b,c,a] /; 
      FreeQ[VectorParameters,RE[a]]\[Equal]False && 
        FreeQ[VectorParameters,RE[b]] \[Equal]False;
MatMul[a_,b_,c_] := 
    MatMul[b,c,a] /; 
      FreeQ[VectorParameters,Re[c]]\[Equal]False && 
        FreeQ[VectorParameters,RE[b]] \[Equal]False;
Tp[-a___]:=-Tp[a];
Adj[-a___]:=-Adj[a];
...
MatMul[d___,-a_,c___][x_,y_]:=-MatMul[d,a,c][x,y]; 
 MatMul[MatMul[a__],b__]:=
    If[Length[getDimParameters[Last[{a}]  /. Conj[x_]\[Rule]x]]\[Equal]1,
      Return[MatMul[Tp[a],b]];,Return[MatMul[a,b]];]; 
(* MatMul[MatMul[a__],b__]:=
      If[Length[getDimParameters[Last[{a}]]]\[Equal]1,
        Return[MatMul[Tp[a],b]];,Return[MatMul[a,b]];]; *)
...
CalcRGEValue[x_,Finish_]:=Block[{temp,i,j},
      temp  = 
        Expand[x] /. {VEVi[a__]\[Rule]0,Yijk[a__]\[Rule]0, Aijk[a__]\[Rule]0,
            Bij[a__]\[Rule]0,Muij[a__]\[Rule]0,m2ij[a__]\[Rule]0, 
            TA[a__]\[Rule]0} ;
      ...
      If[ MakeMatrixMul\[Equal]True,
        temp = 
          temp //. sum[a_,b_,c_] d_ \[RuleDelayed]Sum[d,{a,b,c}] //.subRGE;
        ...
        temp = temp //. sum[a_,b_,c_] d_ \[RuleDelayed]Sum[d,{a,b,c}];
        
        temp = makeSumAllRGE[CalcDelta[CalcEpsRGE[temp]]];
        While[FreeQ[temp,sum]\[Equal]False,
          temp = ReleaseHold[
                temp  /. sum[a_,b_,c_,d_]\[Rule] Hold[Sum[d,{a,b,c}]]];];
        
        temp = CanonicalOrder[temp] /. sumRGE \[Rule]sum;
...
SA`Casimir[a_,b_]:=SA`Casimir[getBlankSF[a],b] /; FreeQ[a,List]\[Equal]False
SA`Dynkin[a_,b_]:=SA`Dynkin[getBlankSF[a],b] /; FreeQ[a,List]\[Equal]False
...
partRGE[particle_,nr_]:=Block[{res,sub,i,temp,ind,sumStates},
        ...
        For[i=1,i\[LessEqual]Length[ind],
          If[ind[[i]]=!=generation&&ind[[i]]=!=lorentz,
            ...
            sumStates=
                sumStates*
                  sum[ind[[i]] /. subGC[nr],1,
                    NumberStates[ind[[i]],particle]];
            ];
          i++;];
        res=
...
TraceValue2[particle_]:=Block[{pos,pos2,temp,traceFac,i,fac,fac2},
        ...
        For[i=1,i\[LessEqual]Length[ind],
          If[ind[[i]]=!=generation&&ind[[i]]=!=lorentz,
            fac=fac*NumberStates[ind[[i]],particle];
            ];
          i++;];
