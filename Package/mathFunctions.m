...
conj[x_]:=x /; (MemberQ[realVar,x] ||MemberQ[realHeads,Head[x]] || 
            MemberQ[realVar,Head[x]] || 
            MemberQ[realHeads,Head[x]])  && (Head[x]=!=
            Pattern) && (Head[Head[x]]=!=Pattern);
conj[x_conj]:=Plus@@x;
...
bar[x_]:=x /; (MemberQ[MajoranaPart,x] ||MemberQ[MajoranaPart,Head[x]] || 
            MemberQ[realHeads,x] || MemberQ[realHeads,Head[x]]) && (Head[x]=!=
            Pattern) && (Head[Head[x]]=!=Pattern);
...
DPF[term_,particle1_,particle2_,fnr_]:=
    Sum[(D[term,
              particle1 /.subGC[i] ]*(particle2[fnr] /. subGC[i]) /. {GetGen[
                getBlank[particle1]] \[Rule]GetGen[getBlank[particle2]],
            GetGenStart[getBlank[particle1]] \[Rule]
              GetGenStart[getBlank[particle2]]}),{i,1,genMax}]; 
DMM[term_,particle_,number__,name_,fnr_]:=Block[{i,j,res, temp, part},
      If[getType[particle]===F,part=getFull[particle][fnr];,
        part=getFull[particle];];
      Return[
        Sum[D[term,part /.subGC[i]]  /. 
              subIndFinalX[i,number,name] /.subFinalX,{i,1,genMax}]];
      ];

DPV[term_,particle_,t_,fermpos_]:=Block[{part,i},
      If[getType[particle]===F,part=getFull[particle][fermpos];,
        part=getFull[particle];];
      If[Length[getIndizes[particle]]\[Equal]0 && (OnesChecked\[Equal]True),
        Return[D[term,part]];,
        Return[
            Plus@@Table[
...
DFTerms[term_,particle_]:=Block[{i,j,res,temp, fact},
      ...
      For[i=1,i\[LessEqual]genMax,
        ...
        LagrangianAux -= (auxP +conj[auxP])\[ImaginaryI];
        i++;];
...
DIntOut[term_,particle_]:=Block[{i,j,res, fact, temp2},
      ...
      For[i=1,i\[LessEqual]genMax,
        temp+= 
          D[term,part  /.subGC[i]] /. subIndFinalX[i,1,"IO"] /. 
              subGCRE[genMax,i] /.sum[gIO1,a_,b_]\[Rule]1;
        i++;];
      temp2=(temp /.subGCRE[1,4] /.subGCRE[2,5] /.subGCRE[3,6]);
      If[getType[particle]===F,
        temp2=temp2 /. {A_[1]\[Rule]A[2], L1\[Rule]L3, L2\[Rule]L4};
        If[
          MemberQ[MajoranaPart,
              getBlank[particle] /.diracSubBack1]\[Equal]True ,
          
          effOp=  temp*
                  conj[temp2]+(conj[
                        temp]/.{A_[1]\[Rule]A[2]})(temp2/.{A_[2]\[Rule]A[1]})+
                temp*temp2+(temp/.{A_[1]\[Rule]A[2]})(temp2/.{A_[2]\[Rule]A[
                            1]});,
          
          effOp = temp*
                  conj[temp2]+(conj[
                        temp]/.{A_[1]\[Rule]A[2]})(temp2/.{A_[2]\[Rule]A[
                            1]}); 
          ];,
        temp2=temp2 /. {A_[1]\[Rule]A[3],A_[2]\[Rule]A[4]};
        If[conj[particle]===particle,
          
          effOp= temp*
                  conj[temp2]+(conj[temp]/.{A_[1]\[Rule]A[2],
                        A_[2]\[Rule]A[1]})(temp2/.{A_[3]\[Rule]A[4],
                        A_[4]\[Rule]A[3]})+ 
                temp*temp2+(temp/.{A_[1]\[Rule]A[2],
                        A_[2]\[Rule]A[1]})(temp2/.{A_[3]\[Rule]A[4],
                        A_[4]\[Rule]A[3]}); ,
...
ParameterQ[x_]:=
    If[FreeQ[Transpose[parameters][[1]],x]\[Equal]True, Return[False];,
      Return[True];];

Delta[a_,a_]:=1;
Delta[a_Integer,b_Integer]:=If[a\[Equal]b,1,0];
Delta[a_,0]:=0;
Delta[0,a_]:=0;  
...
LHBlockName[x_]:=Block[{pos,temp},
      ...
      If[pos==={}, 
        Message[Parameter::NoLesHouches,x];
        Return["None"];
        ];
