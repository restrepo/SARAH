...
GenerateSPhenoBR[Eigenstates_]:=Block[{i},
      ...
      sphenoBR=
        OpenWrite[
          ToFileName[$sarahCurrentSPhenoDir,
            "BranchingRatios_"<>ModelName<>".f90"]];
      ...
      For[i=1,i\[LessEqual]Length[ListDecayParticles3B],
        WriteString[sphenoBR, 
          "Use "<>SPhenoForm[ListDecayParticles3B[[i,1]]] <>"3Decays_"<>
            ModelName<>" \n"];
        i++;];
      ...
      WriteString[sphenoBR, 
        "End Module BranchingRatios_"<>ModelName<>" \n "];
      Close[sphenoBR];
      
      ];

WriteSPhenoBR[Eigenstates_]:=Block[{i},
      MakeSubroutineTitle["CalculateBR",
        Join[Join[NewMassParameters,Join[listVEVs,listAllParameters]],
          SPhenoWidthBR],{"CTBD","fac3","epsI","deltaM","kont"},{},sphenoBR];
      ...
      MakeVariableList[{cplHiggsPP,cplHiggsGG, cplPseudoHiggsPP,
          cplPseudoHiggsGG, cplHiggsZZvirt, cplHiggsWWvirt},"",sphenoBR];
      ...
      (* WriteString[sphenoBR,
            "Real(dp) :: "<>ToString[SPhenoWidth[VectorZ]]<>","<>
              ToString[SPhenoWidth[VectorW]]<>" \n"]; *)
      ...
      WriteString[sphenoBR,
        "CPL_A_HH_Z = "<>ToString[
            SPhenoCoupling[C[VectorZ,HiggsBoson,PseudoScalar]]]<>"**2/("<>
          ToString[leftCoupling]<>"**2/(cos(ThetaW)*4._dp))\n"];
      ...
      For[i=1,i\[LessEqual]Length[savedDecayInfos],
        WriteString[sphenoBR,
          "gP"<>ToString[savedDecayInfos[[i,1]]]<>" = 0._dp \n"];
        WriteString[sphenoBR,
          "gT"<>ToString[savedDecayInfos[[i,1]]]<>" = 0._dp \n"];
        WriteString[sphenoBR,
          "BR"<>ToString[savedDecayInfos[[i,1]]]<>" = 0._dp \n"];
        
        If[
          FreeQ[Transpose[ListDecayParticles3B][[1]],
              savedDecayInfos[[i,1]]]\[Equal]True,
          
          MakeCall[ToString[savedDecayInfos[[i,1]]]<>"TwoBodyDecay",
              Flatten[{savedDecayInfos[[i,2]],
                  savedDecayInfos[[i,3]]}],{"-1"},{"gP"<>
                  ToString[savedDecayInfos[[i,1]]],
                "gT"<>ToString[savedDecayInfos[[i,1]]],
                "BR"<>ToString[savedDecayInfos[[i,1]]]},sphenoBR];,
          
          pos=Position[BR2and3,savedDecayInfos[[i,1]]][[1,1]];
          length2B = "(:,1:"<>ToString[Extract[BR2and3,pos][[2]]]<>")";
          
          length3B = 
            "(:,"<>ToString[Extract[BR2and3,pos][[2]]+1] <>":"<>
          ...
          MakeCall[ToString[savedDecayInfos[[i,1]]]<>"TwoBodyDecay",
            Flatten[{savedDecayInfos[[i,2]],savedDecayInfos[[i,3]]}],{"-1"},{"gP"<>
                ToString[savedDecayInfos[[i,1]]]<>length2B,
              "gT"<>ToString[savedDecayInfos[[i,1]]],
              "BR"<>ToString[savedDecayInfos[[i,1]]]<>length2B},sphenoBR];
          
          If[getGenSPheno[savedDecayInfos[[i,1]]]>1,
            
            WriteString[sphenoBR,
                "If (MaxVal(gT"<>ToString[savedDecayInfos[[i,1]]]<>").Lt.MaxVal(fac3*Abs("<>
                  ToString[SPhenoMass[savedDecayInfos[[i,1]]]]<>
                  "))) Then \n"];,
            
            WriteString[sphenoBR,
                "If (gT"<>ToString[savedDecayInfos[[i,1]]]<>".Lt.fac3*Abs("<>
                  ToString[SPhenoMass[savedDecayInfos[[i,1]]]]<>
                  ")) Then \n"];
            ];
          
          
          pos=Position[Transpose[ListDecayParticles3B][[1]],
                savedDecayInfos[[i,1]]][[1,1]];
          
          MakeCall[ToString[savedDecayInfos3Body[[pos,1]]]<>"ThreeBodyDecay",
            Flatten[{savedDecayInfos3Body[[pos,2]],
                savedDecayInfos3Body[[pos,3]],savedDecayInfos3Body[[pos,4]],
                savedDecayInfos3Body[[pos,5]]}],{"-1"},{"epsI", "deltaM",".False.",
              "gT"<>ToString[savedDecayInfos3Body[[pos,1]]],
              "gP"<>ToString[savedDecayInfos3Body[[pos,1]]]<>length3B,
              "BR"<>ToString[savedDecayInfos3Body[[pos,1]]]<>length3B},
            sphenoBR];
          ...
          MakeCall[ToString[savedDecayInfos3Body[[pos,1]]]<>"ThreeBodyDecay",
            Flatten[{savedDecayInfos3Body[[pos,2]],
                savedDecayInfos3Body[[pos,3]],savedDecayInfos3Body[[pos,4]],
                savedDecayInfos3Body[[pos,5]]}],{"-1"},{"epsI", "deltaM",".True.",
              "gT"<>ToString[savedDecayInfos3Body[[pos,1]]],
              "gP"<>ToString[savedDecayInfos3Body[[pos,1]]]<>length3B,
              "BR"<>ToString[savedDecayInfos3Body[[pos,1]]]<>length3B},
            sphenoBR];
          ...
          WriteString[sphenoBR,
            "Do i1=1,"<>ToString[getGenSPheno[savedDecayInfos[[i,1]]]] <>
              "\n"];
          
          WriteString[sphenoBR,
            SPhenoWidth[savedDecayInfos[[i,1]],i1]<>" =Sum("<>
              ToString[SPhenoPartialWidth[savedDecayInfos[[i,1]]]]<>
              "(i1,:)) \n"];
          
          WriteString[sphenoBR,
            "If ("<>SPhenoWidth[savedDecayInfos[[i,1]],i1]<>".Gt.0._dp) "<>
              ToString[SPhenoBR[savedDecayInfos[[i,1]]]]<>"(i1,: ) ="<>
              ToString[SPhenoPartialWidth[savedDecayInfos[[i,1]]]] <>"(i1,:)/"<> 
              ToString[SPhenoWidth[savedDecayInfos[[i,1]],i1]]<>" \n"];
