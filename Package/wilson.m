...
InitDiagramGeneration:=Block[{i,j,k,l,temp,pos},
      ...
      temp =Join[VertexList3,VertexList4]/. a_[{b__}]\[Rule]a;
      
      
      For[i=1,i\[LessEqual]Length[temp],
        For[j=1,j\[LessEqual]Length[temp[[i,1]]],
          ...
          For[k=j+1,k\[LessEqual]Length[temp[[i,1]]],
            If[FreeQ[TwoKnownIns,C[temp[[i,1,j]],temp[[i,1,k]]]],
              TwoKnown = Join[TwoKnown,{Delete[temp[[i,1]],{{j},{k}}]}];
              
              TwoKnownIns= 
                Join[TwoKnownIns,{C[temp[[i,1,j]],temp[[i,1,k]]]}];,
              
              pos= Position[TwoKnownIns,C[temp[[i,1,j]],temp[[i,1,k]]],1,
                    1][[1,1]];
              
              TwoKnown[[pos]] = 
                Join[TwoKnown[[pos]],Delete[temp[[i,1]],{{j},{k}}]];
              ];
            For[l=k+1,l\[LessEqual]Length[temp[[i,1]]],
              
              If[FreeQ[ThreeKnownIns,
                  C[temp[[i,1,j]],temp[[i,1,k]],temp[[i,1,l]]]],
                
                ThreeKnown = 
                  Join[ThreeKnown,{Delete[temp[[i,1]],{{j},{k},{l}}]}];
                
                ThreeKnownIns= 
                  Join[ThreeKnownIns,{C[temp[[i,1,j]],temp[[i,1,k]],
                        temp[[i,1,l]]]}];,
                
                pos= Position[ThreeKnownIns,
                      C[temp[[i,1,j]],temp[[i,1,k]],temp[[i,1,l]]],1,1][[1,
                    1]];
                
                ThreeKnown[[pos]] = 
                  Join[ThreeKnown[[pos]],Delete[temp[[i,1]],{{j},{k},{l}}]];
                ];
              l++;];
            k++;];
          j++;];
...
AddFieldToDiagram[process_]:=
    Block[{pos, possFields,i,possFieldsSub, newProcess},
      currentInsertion = Cases[process,FieldToInsert[_],3,1][[1]];
      pos = Position[process[[1]],currentInsertion];
      For[i=1,i\[LessEqual]Length[pos],
        currentVertex=Extract[process[[1]],pos[[i,1]]];
        If[Length[pos[[i]]]\[Equal]2,
          If[i\[Equal]1,
              possFields=getPossibleFields[currentVertex];,
              
              possFields=
                  Intersection[possFields,getPossibleFields[currentVertex]];
              ];,
          If[i\[Equal]1,
              possFields=AntiField/@getPossibleFields[currentVertex];,
              
              possFields=
                  Intersection[possFields,
                    AntiField/@getPossibleFields[currentVertex]];
              ];
          ];
        i++;];
      If[Length[possFields]>0,
        possFieldsSub=
          Flatten[Table[{currentInsertion\[Rule]possFields[[i]]},{i,1,
                Length[possFields]}]];
        For[i=1,i\[LessEqual]Length[possFieldsSub],
          newProcess= process /. possFieldsSub[[i]];
          Switch[CheckVertex[newProcess],
            Finished,
            		If[FreeQ[ListInsertedVertices,newProcess[[1]]],
                		DiagramList = Join[DiagramList,{newProcess}];
                		
                ListInsertedVertices=
                  Join[ListInsertedVertices,{newProcess[[1]]}];
                		];,
            GoOn,
...
CheckVertex[process_]:=Block[{goOn,i},
      ...
      If[Length[finishedVertices]\[Equal]0,
        Return[GoOn];,
        goOn = True;
         For[i=1,i\[LessEqual]Length[finishedVertices],
          If[FreeQ[VerticesInv[All],finishedVertices[[i]]],
            goOn=False;
            ];
          i++;]; 
        If[goOn\[Equal]True,
          If[Length[finishedVertices]\[Equal]Length[process[[1]]],
              Return[Finished];,
              Return[GoOn];
              ];,
          Return[stop];
...
getPossibleFields[vertex_]:=Block[{temp,pos},
      ...
      Switch[Length[temp],
        0, Return[OneKnownIns /. C\[Rule]Plus];,
        1,
        	pos=Position[OneKnownIns,temp,1,1][[1,1]];
        	Return[Extract[OneKnown,pos]];,
        2,
        	pos=Position[TwoKnownIns,temp,1,1][[1,1]];
        	Return[Extract[TwoKnown,pos]];,
        3,
        	pos=Position[ThreeKnownIns,temp,1,1][[1,1]];
        	Return[Extract[ThreeKnown,pos]];
        ];
      ];
...
GenerateBoxDiagrams[f1_,f2_,f3_,f4_]:=Block[{},
      ...
      currentTop={{C[f1,FieldToInsert[1],AntiField[FieldToInsert[4]]],
            ...
            C[f3,AntiField[FieldToInsert[2]],FieldToInsert[3]],
            C[f2,AntiField[FieldToInsert[3]],
              FieldToInsert[4]]},{Internal[1]\[Rule]FieldToInsert[1],
            Internal[2]\[Rule]FieldToInsert[2],
            Internal[3]\[Rule]FieldToInsert[3],
            Internal[4]\[Rule]FieldToInsert[4],External[1]\[Rule]f1,
            External[2]\[Rule]f4,External[3]\[Rule]f3,External[4]\[Rule]f2,
            Index[1]\[Rule]gt1,Index[2]\[Rule]gt4,Index[3]\[Rule]gt3,
            Index[4]\[Rule]gt2, InsertionOrder \[Rule]3}};
      InsFields[currentTop];
      AllBoxDiagrams=Join[AllBoxDiagrams,InsFields[currentTop]]; 
      
      currentTop={{C[f1,FieldToInsert[1],AntiField[FieldToInsert[4]]],
            C[f3,AntiField[FieldToInsert[1]],FieldToInsert[2]],
            ...
            Internal[3]\[Rule]FieldToInsert[3],
            Internal[4]\[Rule]FieldToInsert[4],External[1]\[Rule]f1,
            External[2]\[Rule]f3,External[3]\[Rule]f2,External[4]\[Rule]f4,
            Index[1]\[Rule]gt1,Index[2]\[Rule]gt4,Index[3]\[Rule]gt3,
...
GeneratePenguinDiagrams[f1_,f2_,f3_]:=Block[{},
      currentTop={{C[f1,FieldToInsert[1],AntiField[FieldToInsert[2]]],
            C[f2,AntiField[FieldToInsert[1]],FieldToInsert[3]],
            C[f3,AntiField[FieldToInsert[3]],
              FieldToInsert[2]]},{Internal[1]\[Rule]FieldToInsert[1],
            Internal[2]\[Rule]FieldToInsert[2],
            Internal[3]\[Rule]FieldToInsert[3],External[1]\[Rule]f1,
            External[2]\[Rule]f2,External[3]\[Rule]f3}};
